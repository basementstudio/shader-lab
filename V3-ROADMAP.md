...
Etapa 2 (registrada, no prometida): efectos por distancia. La capa Image renderiza además la profundidad desplazada a un destino auxiliar y la pipeline la ofrece a la pila como **profundidad de escena**; las máscaras de capa suman una forma **Depth** (rango cerca/lejos con Feather) para limitar cualquier efecto, y Gradient Map puede colorear por distancia. Mientras tanto, Show Depth más una capa duplicada en **Mode → Mask** cubre el mismo uso a mano. Profundidad en video (un segundo video alineado) y generación automática siguen como decisiones separadas.
