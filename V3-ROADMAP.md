...
**Signal Rot implementado (23 de septiembre de 2026), pendiente de prueba del usuario.** Capa de efecto **Signal Rot** (catálogo Distort, tras Slice), un solo pase que reubica el muestreo en el marco de las rayas (Streaks: Vertical / Horizontal): bandas rasgadas con bordes irregulares que se desplazan (Tear, Band Size) y caen al color de papel (Dropout, Dropout Color), ondulación lateral (Wobble, Wobble Scale), velocidad de escaneo irregular (Stretch), segmentos retenidos con frente ondulado que arrastran la imagen en rayas largas (Drag, Drag Length), salto de líneas de escaneo y ganancia por línea (Line Noise), desplazamiento de rojo y azul (Chroma Shift) y cuantización de niveles (Crush); Speed 0 congela el daño, mayor a 0 lo hace pudrirse en el tiempo; Seed. Cuatro estilos: Scanner Drag (por defecto, referencia 13), Signal Rot, Tape Wobble y Torn Scan. Se distingue de Slice (bloques), CRT (pantalla) y Circuit Bent (líneas curvadas). Referencia 13 revisada con renders de cada estilo: Scanner Drag y Torn Scan logran las rayas retenidas, los frentes ondulados y las roturas blancas; la riqueza de rayas depende de la foto (un cielo liso da rayas planas). Sin realimentación entre frames. [Prueba manual](tests/composition/SIGNAL-ROT-MANUAL-QA.md).
