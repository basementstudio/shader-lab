...
**Implementado, pendiente de aceptación visual del usuario:** cada capa (grupos incluidos) tiene una máscara opcional con forma None / Linear gradient / Radial gradient / Ellipse / Rectangle / Brush, alcance **Limit effect** / **Cut content** (solo efectos; en fuentes y grupos ambos equivalen a recortar la propia capa), Invert, Enabled, centro, tamaño, rotación y Feather (elipse y rectángulo). Manijas directas en el lienzo: centro, eje X (rotación y ancho), eje Y (alto); el gradiente lineal usa inicio/fin. El pincel reutiliza la interacción de Cells (Reveal/Erase, Brush Size, Clear, un trazo por acción de historial, entrada automática al elegir Brush) con el mismo códice compacto, sin acoplarse a la geometría de Cells. Coordenadas centradas en unidades del lado corto, iguales a Paint; la máscara sigue el viewport hasta resolver el artboard estable (2.8). Interpolación premultiplicada sin halos negros; el modo antiguo **Mode → Mask** no cambia. Editor y runtime comparten `renderer/layer-mask.ts`; mover una máscara solo actualiza uniforms. Persistencia .lab, duplicado, historial, export PNG/video, runtime y MCP `update_layer.mask`. Referencias revisadas: 17 y 20. [Prueba manual](tests/composition/LAYER-MASKS-MANUAL-QA.md). Máscaras animables por timeline: implementadas el 23 de septiembre de 2026 (ver 6.4). Feather en máscaras de pincel: pedido por el usuario el 23 de septiembre de 2026, implementado (tres anillos de muestras en GPU, animable); con 0 el borde queda duro. Pendiente: selección de una capa de formas como máscara (2.7).
...
**Formas, jitter y líneas en anillos (23 de septiembre de 2026, pendiente de prueba del usuario).** El usuario sentía que a la capa le faltaba algo frente a la referencia 16. Releída: cada banda es la misma foto girada un ángulo propio e irregular (no una espiral regular), los anchos varían y líneas finas concéntricas marcan cada borde y siguen por todo el afiche negro más allá de la foto. Se añadieron Shape (Circle, Triangle, Square, Polygon con Sides; el control anterior pasa a llamarse Cut: Full / Half), Rotation Jitter y Width Jitter por banda con Seed, y Lines (None, Band edges, Edges and beyond) con ancho, color y opacidad. Todo apagado por defecto: las capas y proyectos existentes no cambian. Estudios con Threshold azul sobre negro dentro del grupo reproducen el lenguaje de 16 con círculos; cuadrados y triángulos dan marcos girados (con polígonos conviene Band edges). La textura de puntos de 16 puede venir de Dot Grid o Pattern. La ampliación de formas propuesta abajo queda implementada. [Prueba manual](tests/composition/DISPLACED-RINGS-MANUAL-QA.md).
...
| **Dot Grid** | La trama de puntos de 6.1 #2 (afiche celeste, `04-trama-puntos-halos.png`): grilla ortogonal exacta fijada al artboard, un punto en cada celda que crece con el tono, bordes que se disuelven en puntos chicos y una copia difusa debajo; sin ángulos ni CMYK | 3 |
| **Erosion** | La erosión de 6.1 #2 (`09-collage-tipografia-tramas.png`, `14-rostro-rosa-erosionado.png`): la silueta de la foto se desintegra en motas de píxeles con fragmentos dispersos; hacia papel o transparencia | 4 |
| Posteriores | Flares (implementado; Halos de color descartado), Frequency modulation, Retrato de puntos conectados, Relieve grabado (6.1 #5, puede usar profundidad de 6.4) | 5 |

**Lumen Print implementado (23 de septiembre de 2026); el usuario lo probó en el dev server: "se ve increíble".** Capa de efecto **Lumen Print** (catálogo Core, tras Gradient Map), un solo pase: 16 muestras en espiral áurea con giro por píxel para Diffusion y Halation (Spread en píxeles del documento), cruz de 4 muestras para Edge Lines, Exposure/Contrast, Solarize con Pivot (pliegue alrededor de un tono: Pivot bajo tiende al negativo, alto invierte solo las luces), Washout hacia el extremo papel de la paleta con Ragged Edge, Edge Burn en los bordes de la hoja y Grain/Grain Size en los medios tonos, con Seed; al final la LUT lineal compartida con Gradient Map. Seis estilos que escriben todos los controles: Lumen (por defecto), Cyanotype, Burned (07), Sabattier (12), Washed (06) y Eroded (14); editar cualquier control muestra Custom. Referencias 06, 07, 10, 12 y 14 revisadas con renders de cada estilo sobre fotografía: Burned, Sabattier y Washed se acercan a 07, 12 y 06; Eroded logra la erosión hacia el papel pero queda demasiado binario frente a 14 (conserva poco detalle tonal dentro del rosa). Pendiente: aceptación final y ajuste de valores iniciales por el usuario, grano animado para video, medición en hardware nativo. [Prueba manual](tests/composition/LUMEN-PRINT-MANUAL-QA.md).

**Signal Rot implementado (23 de septiembre de 2026), aceptado visualmente por el usuario el 23 de septiembre de 2026 ("se ven increíbles"), que pidió quitar el estilo Tape Wobble.** Capa de efecto **Signal Rot** (catálogo Distort, tras Slice), un solo pase que reubica el muestreo en el marco de las rayas (Streaks: Vertical / Horizontal): bandas rasgadas con bordes irregulares que se desplazan (Tear, Band Size) y caen al color de papel (Dropout, Dropout Color), ondulación lateral (Wobble, Wobble Scale), velocidad de escaneo irregular (Stretch), segmentos retenidos con frente ondulado que arrastran la imagen en rayas largas (Drag, Drag Length), salto de líneas de escaneo y ganancia por línea (Line Noise), desplazamiento de rojo y azul (Chroma Shift) y cuantización de niveles (Crush); Speed 0 congela el daño, mayor a 0 lo hace pudrirse en el tiempo; Seed. Tres estilos: Scanner Drag (por defecto, referencia 13), Signal Rot y Torn Scan. Se distingue de Slice (bloques), CRT (pantalla) y Circuit Bent (líneas curvadas). Referencia 13 revisada con renders de cada estilo: Scanner Drag y Torn Scan logran las rayas retenidas, los frentes ondulados y las roturas blancas; la riqueza de rayas depende de la foto (un cielo liso da rayas planas). Sin realimentación entre frames. [Prueba manual](tests/composition/SIGNAL-ROT-MANUAL-QA.md).

**Decisión del usuario (23 de septiembre de 2026):** Halftone no se modifica. Es una simulación de impresión (ángulos, CMYK, ganancia de punto) y los puntos buscados son perfectos, casi de grilla; por eso la trama y la erosión son capas nuevas independientes, primero Dot Grid y luego Erosion.

**Dot Grid implementado y aceptado visualmente por el usuario (23 de septiembre de 2026, "se ve muy bien").** Capa de efecto **Dot Grid** (catálogo Core, tras Halftone): grilla cuadrada exacta centrada en el artboard (Spacing en píxeles del documento), un punto por celda cuyo radio va de Min Dot a Max Dot según el tono leído con cinco muestras alrededor del centro (Softness), con Level (tono donde empieza a crecer) y Contrast; círculo o cuadrado con antialias, tinta de un color o del color de la fuente, fondo y copia difusa debajo (Underlay, Underlay Blur), Invert. Cuatro estilos: Coordinate (por defecto, referencia 04), Source Dots, Night y Pixel Grid. Referencia 04 revisada con renders: la primera versión daba puntos medianos en el cielo claro; Level permite que el papel vacío quede en la mota mínima y los puntos crezcan solo en las formas oscuras, como en 04. Sin ángulos, CMYK ni jitter; los destellos en cruz de 04 quedan para Halos de color. [Prueba manual](tests/composition/DOT-GRID-MANUAL-QA.md). Sigue Erosion.

**Erosion implementado y aceptado por el usuario (23 de septiembre de 2026, "suficientemente bueno").** Capa de efecto **Erosion** (catálogo Distort, tras Signal Rot), con rol de composición `transform` para poder recortar cobertura: un campo de erosión leído con cinco muestras (Erode From: Edges / Light tones / Dark tones / Cutout edge, el borde de imágenes transparentes o texto), bloques de motas (Speckle Size, Clumping) que se eliminan según Erode, Edge Width, y Scatter que arroja fragmentos a través del borde conservando su color. Reveal: Paper (Paper Color) o Transparent (dentro de un grupo muestra lo que hay debajo). Speed y Seed. Tres estilos: Disintegrate (por defecto, referencia 09), Paper Erosion (14) y Crumbled Cutout (texto). Referencias 09 y 14 revisadas con renders: la primera versión solo abría huecos blancos y detectaba toda la textura; se añadieron fragmentos que sobreviven y se exigieron contornos más fuertes, y Disintegrate ahora desintegra las siluetas en motas con migas de color; Crumbled Cutout desmorona letras sobre una foto. Lumen Print (Washout) sigue siendo la versión suave. [Prueba manual](tests/composition/EROSION-MANUAL-QA.md). Con esto quedan cubiertas las tres capas consolidadas y la separación de 6.1 #2; siguen las posteriores (Halos de color, Frequency modulation, Retrato de puntos conectados, Relieve grabado) o la fase 4.

**Relieve grabado implementado como Relief (23 de septiembre de 2026), aceptado por el usuario el 23 de septiembre de 2026 ("bastante decente"), que pidió emboss y deboss.** Capa de efecto **Relief** (catálogo Core, tras Dot Grid): altura desde Luminance, Depth (profundidad de escena de 6.4) o Cutout (alfa de texto e imágenes transparentes), **Emboss / Deboss**, Depth, Bevel, luz direccional (Light Angle, Elevation, Ambient), superficie de un color o con los colores de la fuente, Specular y Shininess, Grain, y grabado opcional en líneas paralelas o estallido radial (más profundo en los oscuros). Cinco estilos: Silver Plate (por defecto, referencia 15), Letterpress (deboss sobre papel), Blind Emboss, Gold Foil y Source Relief. Referencia 15 revisada con renders: Silver Plate logra la placa plateada con estallido radial y grano; el centro del estallido producía moiré y ahora se desvanece. Letterpress revisado sobre texto agrupado. [Prueba manual](tests/composition/RELIEF-MANUAL-QA.md).

**Halos de color descartados; se conserva Flares (23 de septiembre de 2026, decisión del usuario).** Tras probar el prototipo Color Halos, el usuario solo conservó los destellos en cruz (composición propia de una calavera con cruces naranjas en los brillos) y pidió quitar el resto: los halos no combinaban con los destellos y por ahora no les ve uso. La capa pasa a llamarse **Flares** (catálogo Core, tras Lumen Print), con rol `transform`: detección de puntos brillantes aislados a un cuarto de resolución (Threshold, Isolation, Thickness), rayos acumulados por pixel a lo largo de cada dirección con tantas muestras como pide su longitud (Rays 1–16, Rotation, Length, Secondary Rays, Length Jitter, Seed, Falloff), color de rayo y de núcleo, Core Glow y Core Size, compuesto en modo pantalla. Cinco estilos: Cross (por defecto, los valores del usuario), Star, Starburst, Asterisk y Anamorphic. Renders revisados: la primera versión dejaba rayos punteados (pocas muestras) y un error de nombres en los bucles anidados de TSL; ambos corregidos. Halos de color (6.1 #3) queda fuera de V3 salvo que el usuario lo retome. El usuario aceptó Flares el 23 de septiembre de 2026 ("mucho más útil"). [Prueba manual](tests/composition/FLARES-MANUAL-QA.md).

**Desenfoque y vidrio (23 de septiembre de 2026, decisión del usuario).** El usuario considera "terribles" Directional Blur, Progressive Blur y Fluted Glass (por cómo se ven). Se reemplazan por dos capas nuevas sobre un mismo motor de desenfoque de calidad (pirámide de niveles con muestreo B-spline; radios grandes suaves y tamaño variable por píxel): primero **Blur** (Blur From: Uniform / Depth / Linear / Radial / Luminance; Gaussian, Lens o Motion; grano), con la referencia 31; después **Glass** (Reeded, Hammered, Pyramid, Hex, Frosted; cada celda es una lente, desenfoque según la distancia detrás del vidrio, brillos en los bordes, textura escarchada), con las referencias 26 a 30. Las tres capas viejas salen del menú de agregar pero siguen funcionando en proyectos guardados. **Blur implementado y aceptado por el usuario (23 de septiembre de 2026, "se ve increíble"):** capa **Blur** (catálogo Core, tras Flares), rol `transform`, pirámide de siete niveles con muestreo B-spline; Blur From Uniform / Depth / Linear / Radial / Luminance con Focus, Focus Range, Transition, Center, Band Angle e Invert Focus; Kind Gaussian, Lens (bokeh con Highlights) o Motion; Grain, Grain Size y Grain in Blur. Cinco estilos: Depth of Field (por defecto, referencia 31), Tilt-Shift, Vignette, Soft y Motion. Un estudio sintético de tubos y esferas negras sobre amarillo con mapa de profundidad reproduce el lenguaje de 31. Directional Blur, Progressive Blur y Fluted Glass ya no aparecen en el menú. [Prueba manual](tests/composition/FOCUS-BLUR-MANUAL-QA.md). **Glass implementado y aceptado por el usuario (23 de septiembre de 2026, "me encanta"):** capa **Glass** (catálogo Distort), rol `transform`, sobre la pirámide compartida con Blur: patrones Reeded (perfil redondo o prisma), Hammered, Pyramid, Hex y Frosted; cada celda es una lente (Refraction: poco aumenta, 0.5 enfoca en una línea, 1 invierte en mini imágenes), Distance y Distance From (Uniform o Depth: lo cercano al vidrio queda nítido), bordes finos iluminados, ranuras, escarcha, dispersión y tinte. Seis estilos: Reeded, Frosted Reeded, Hammered, Pyramid, Hex y Frosted. Referencias 26 a 30 revisadas con renders: la primera versión parecía plástico (sombreado de cilindro, escarcha brillante, celdas martilladas como burbujas); se redujo el sombreado a bordes finos y ranuras de ancho fijo, las celdas martilladas pasaron a facetas poco profundas y la escarcha a un rizado sutil. El estudio con profundidad reproduce el helecho de 29. [Prueba manual](tests/composition/GLASS-MANUAL-QA.md).

**Connected Dots implementado y aceptado por el usuario (23 de septiembre de 2026, "absolutamente perfecto").** Retrato de puntos conectados (referencia 21) como capa **Connected Dots** (catálogo Core, tras Dot Grid), rol `transform`: puntos con jitter por celda que leen tono y color; enlaces con sus 8 vecinos cuyo ancho y probabilidad crecen con la oscuridad. Modos Graph (unión nítida), Blobs (mínimo suave: masas de tinta) y Plexus (líneas finas que se desvanecen). Forma del punto (círculo, cuadrado, cruz, anillo), tamaño por tono, Cutoff para vaciar el fondo, Invert; color por paleta en bandas planas (hasta 8 paradas), de la fuente o tinta; fondo color, imagen o transparente; Drift y Speed para que los enlaces se formen y rompan. Cuatro estilos: Portrait Graph (por defecto, 21), Ink Blobs, Plexus y Constellation. Primeros renders revisados: los enlaces demasiado anchos llenaban las sombras de un bloque sólido; se ajustaron para que queden grafos con huecos como en 21. Segunda etapa (23 de septiembre de 2026, pedida por el usuario): modo **Mesh** (triángulos planos entre los puntos, con Fill, Wire y Wire Color) y seis estilos nuevos: Molecule, Circuit, Neural, Riso, Low Poly y Wireframe; pendiente de prueba. Queda abierto ajustar los puntos a los bordes. [Prueba manual](tests/composition/CONNECTED-DOTS-MANUAL-QA.md).
...
**Parte 1 implementada, pendiente de confirmación del usuario (22 de septiembre de 2026):** el tracker conserva hasta 64 celdas de borde por blob (grilla de análisis 64×36) y las expone como `edge`; el pase añade **Frame** (Outline / Corner brackets / None, con Bracket Length), **Edge Dots** (densidad y tamaño, muestreo por semilla, mismo adelanto por velocidad que las cajas) y **Label Text** (Coordinates / Prefix + ID / Custom list, con Label Seed): los códigos y las palabras se asignan por hash determinista del id de la pista, así no parpadean entre frames. El atlas de etiquetas incorpora letras y puntuación (20 caracteres por etiqueta). `showOutline` queda oculto y migra a `frameStyle` al abrir archivos antiguos. Editor y runtime idénticos salvo imports; escenario `--blob` en el benchmark de video. Referencia 19 revisada. [Prueba manual](tests/composition/BLOB-TRACKING-MANUAL-QA.md). Tras la primera prueba del usuario con video (22 de septiembre): detección de movimiento mejorada —diferencia dilatada en radio 2, grilla de análisis 128×72 (antes 64×36; el ancho debe mantener filas de 256 bytes para la lectura asíncrona), histéresis por celda y fallback a luminancia solo con menos de dos celdas en movimiento—, medida con `tests/composition/blob-motion-probe.mjs` sobre aura.mp4 (área media de blob 0,3 % → 2,4 % de la grilla, celdas en movimiento 1,2 % → 8,8 %, IDs 14 → 11 en 85 frames); nuevo **Lock 1:1** para cuadrados/círculos (activo por defecto en capas nuevas; las escenas guardadas conservan aspecto libre); Trail Decay por defecto 0. Pendiente para la parte 2: colores por elemento, capa Annotations.

**Cierre de la parte 1 (22 de septiembre, tras el segundo test con video):** el usuario confirma que la detección "funciona mucho mejor" y considera que Blob Tracking no admite mucha más mejora; la parte 2 pasa a ser la prioridad. Dos aportes del usuario para la parte 2: (a) reabre `03-rana-falso-color.png` como referencia principal de la capa Annotations: círculos concéntricos y anillos punteados alrededor de un punto de interés con retículo, cruces pequeñas, reglas con marcas y contador (`134`), etiquetas de estado (`NOT FOUND`), lecturas de coordenadas/ángulo (`-{0}2223°`, `14.3`), bloques de metadatos tipográficos en las esquinas (`START 1.000 MHz / STOP 403.000 MHz`, `OP-ID`, `LOC: [REDACTED]`), una barra de degradado, letras aisladas y grietas/ruido orgánico; todo en un solo color tenue sobre falso color; (b) **pincel para decidir dónde se dibujan los elementos**: reutilizar la cobertura pintada de 1.4 como máscara de colocación (además de bordes y semilla), de modo que los elementos sólo aparezcan dentro de la pintura.

**Decisiones finales para la parte 2 (22 de septiembre):** el punto de interés del objetivo se coloca con una manija arrastrable **y** puede ajustarse automáticamente al borde más fuerte; los textos parten de presets de instrumento **editables** línea a línea; el usuario pide especialmente elementos **punteados/discontinuos** (anillos y cajas dashed, reglas, conectores). Colocación: aleatoria con semilla, guiada por bordes y **pintada con pincel**. Colores: mono o paleta por elemento con el editor de rampa.

**Parte 2 implementada y aceptada visualmente por el usuario (22 de septiembre de 2026, con dos composiciones propias: referencias 22 y 23):** capa de efecto **Annotations** (catálogo Core). Trazado determinista en CPU (`renderer/annotations-layout.ts`, compartido por editor y runtime) → hasta 320 marcas y 1024 glifos dibujados como quads instanciados con SDFs (punto, anillo, anillo punteado, retículo, cruz, caja, caja punteada, esquinas, línea, línea punteada, regla con ticks, barra de degradado) y el atlas mono de Blob Tracking para el texto. Colocación **Seeded / Edges / Painted**: bordes por Sobel 128×72 leído cada tres frames; pintada con el mismo pincel y códice `pc1` de máscaras y Cells (target `annotations` del store de pintura). Objetivo con anillos concéntricos sólidos y punteados, retículo, lecturas de ángulo y valor, conectores punteados; manija arrastrable (centro + tamaño) y **Snap to Strongest Edge**. Familias activables (dots, rings, crosses, boxes, connectors, labels, rulers, metadata), Density, Seed, Drift (movimiento con semilla en el tiempo), Scale, Stroke Width, Text Size. Texto: presets Instrument / Surveillance / Scientific / Minimal que rellenan **Words** y **Metadata**, ambos editables. Color mono o **Palette** (rampa; cada elemento elige una parada por semilla). Referencias 03, 10, 19 revisadas; nada se reconoce, y la interfaz lo dice. [Prueba manual](tests/composition/ANNOTATIONS-MANUAL-QA.md). Pendiente: puntos conectados (ref. 21), colocación guiada por regiones/segmentación, rotación de elementos.
...
**Diseño acordado (23 de septiembre de 2026):** la profundidad vive **en la capa Image**, no en una capa de efecto aparte. El usuario adjunta un segundo archivo (mapa de profundidad en escala de grises) a la misma capa; la capa conserva su asset de color y suma `depthAssetId`. El razonamiento del usuario: la profundidad interesa sobre todo **combinada con otros efectos**, así que debe ser un dato de la fuente que el resto de la pila pueda leer, no un efecto cerrado.

Etapa 1 (esta entrega, prototipo de viabilidad): parallax con oclusión en la propia capa Image. El sombreador marcha por capas de profundidad desde lo cercano hacia lo lejano (parallax occlusion mapping, refinado binario), como el bucle de dos fases de DepthFlow, sin depender de esa biblioteca. Controles del grupo **Depth**, visibles solo con un mapa adjunto: Invert Depth (por defecto blanco = cerca), Depth Range (separación entre planos), Focus Plane (profundidad que permanece quieta), Motion (Off / Orbit / Sway / Nod / Dolly), Amount, Speed, Offset (vec2 animable por timeline para una cámara propia), Edges (Stretch: los bordes se estiran; Transparent: alfa 0 fuera del cuadro), Quality (pasos de marcha) y Show Depth (muestra la profundidad ya desplazada en lugar del color). Alineación: el mapa debe compartir encuadre con la foto; se muestrea con las mismas UV y no se realinea. Huecos de desoclusión: se estiran los píxeles vecinos, como hace DepthFlow sin inpainting; no se inventa contenido. El camino sin profundidad no cambia de sombreador ni de coste.

Etapa 2, **profundidad de escena (aprobada por el usuario tras mostrar una composición donde duplicaba la imagen con Show Depth para alimentar ASCII; implementada y aceptada el 23 de septiembre de 2026; el usuario confirma que el frame rate se mantiene con profundidad activa):** la capa Image con mapa resuelve la marcha una sola vez en un destino auxiliar (RGB = profundidad desplazada, blanco = cerca; A = dentro del cuadro) y la pipeline lo lleva hacia arriba como profundidad de escena, también dentro y fuera de grupos; gana la fuente con profundidad más cercana por debajo, y las fuentes sin mapa la dejan pasar. Dos consumidores: (1) **Input: Luminance / Depth** en ASCII, Halftone, Pattern y Gradient Map: el efecto muestrea la profundidad como si fuera la imagen, pero compone sobre el color; sin mapa debajo vuelve a luminancia. (2) Forma de máscara **Depth** con Near / Far / Feather para limitar cualquier efecto por distancia; sin mapa la máscara no hace nada. Coste cero sin profundidad: mismo sombreador, solo cambia qué textura se enlaza, y el cambio de fuente es estructural. Medición en el arnés (SwiftShader, 1080p, mediana por frame): {"plain":32.3,"depthMedium":714.8,"depthHigh":1214.1,"depthLow":433.6} ms; hardware nativo pendiente. Detalle técnico: TSL comparte enlaces de textura por uuid, así que el cambio de fuente recompila el nodo de efecto cuando aparece o desaparece la profundidad. Profundidad en video sigue como decisión separada.

**Máscaras animables (23 de septiembre de 2026, pedidas por el usuario para el barrido de contornos):** Center, Size (Near / Far en la forma Depth), Rotation y Feather de la máscara se exponen al timeline como pistas de parámetro con clave `mask.*`, con botón de keyframe junto a cada control, valores evaluados en editor y runtime, y enlaces de audio. Near y Far son pistas independientes (**Mask Near**, **Mask Far**), por pedido del usuario. El barrido: dos keyframes en cada una y la franja térmica recorre el sujeto de delante a atrás.

Prueba: pares sintéticos color + profundidad generados con Three en el arnés (el usuario no dispone de fotos con mapa), paridad editor/runtime/PNG, hidratación de proyectos anteriores sin cambios de apariencia, y una medición del coste del marcheo a 1080p. La decisión de viabilidad se registra al cierre de la etapa 1.

**Etapa 1 implementada y aceptada por el usuario (23 de septiembre de 2026, con las referencias 24 y 25):** `depthAssetId` en la capa (archivo .lab, historial, duplicado, exportación como `depthAsset`, MCP), sección **Source → Depth map** con Attach / Replace / Remove, grupo **Depth** visible solo con mapa adjunto, marcha en `renderer/media-pass.ts` (editor y runtime) dentro de una `Fn` de TSL con refinamiento binario de cinco pasos; Low / Medium / High = 16 / 32 / 64 capas. El camino sin mapa conserva el sombreador anterior. Comportamiento verificado con pares sintéticos: el borde delantero se desplaza exactamente shift × range × depth; el borde trasero de un sujeto cercano se estira sobre el hueco (el mismo artefacto que DepthFlow sin inpainting; los mapas con bordes suaves lo disimulan). Sin fotos con profundidad del usuario todavía: la aceptación visual necesita una foto real con su mapa. [Prueba manual](tests/composition/DEPTH-PARALLAX-MANUAL-QA.md). Pendiente: coste medido a 1080p en hardware nativo, etapa 2 (profundidad de escena para máscaras y Gradient Map), profundidad en video.

**Generación en la app (23 de septiembre de 2026, pedida por el usuario: no quiere adjuntar mapas, quiere generarlos):** botón **Estimate** junto a Attach. Ejecuta Depth Anything V2 Small (el mismo modelo que usa DepthFlow) en un Web Worker con transformers.js: WebGPU en fp32 cuando hay adaptador, WASM en q8 si no; descarga del modelo desde Hugging Face (99 MB o 27 MB) con caché del navegador y progreso visible; salida en escala de grises con blanco = cerca, reescalada al tamaño de la foto y adjuntada como asset local `nombre-depth.png` (Invert Depth queda en off). Sin inferencia por frame ni video. En Chromium headless con SwiftShader: 10 s de descarga y unos 90 s de inferencia sobre flora.webp; en hardware nativo se espera segundos. La aceptación visual ya puede hacerse con cualquier foto del usuario.
