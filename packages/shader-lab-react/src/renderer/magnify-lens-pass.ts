...
export class MagnifyLensPass extends PassNode {
  ...
  protected override buildEffectNode(): Node {
    ...
    const lensUv = vec2(
      center.x.add(
        renderTargetUv.x
          .sub(center.x)
          .div(mix(float(1), this.zoomUniform, mask))
      ),
      center.y.add(
        renderTargetUv.y
          .sub(center.y)
    ...
    const rim = clamp(
      mask.mul(float(1).sub(mask)).mul(float(4)),
      float(0),
      float(1)
    )

    const radialDirX = renderTargetUv.x
      .sub(center.x)
      .div(dist.add(float(0.0001)))
    const radialDirY = renderTargetUv.y
      .sub(center.y)
      .div(dist.add(float(0.0001)))
  ...
  private handlePointerMove(
    ...
    this.targetX = localX / rect.width
    this.targetY = localY / rect.height
  }
}
