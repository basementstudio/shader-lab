...
export const permute = Fn(([x]: [TSLNode]) => {
  return mod(mul(add(mul(x, 34.0), 10.0), x), 289.0)
})

export const taylorInvSqrt = Fn(([r]: [TSLNode]) => {
  return sub(1.79284291400159, mul(0.85373472095314, r))
})

export const mod289 = Fn(([x]: [TSLNode]) => {
  return mod(mul(add(mul(x, 34.0), 10.0), x), 289.0)
})

export const fade = Fn(([t]: [TSLNode]) => {
  return t
    .mul(t)
    .mul(t)
    .mul(t.mul(t).mul(6.0).sub(t.mul(15.0)).add(10.0))
})

export const grad4 = Fn(([j, ip]: [TSLNode, TSLNode]) => {
