...
`color-halos` runs in the `transform` role so halos can add coverage past a shape. It renders a glow mask (thresholded darkness, lightness or alpha, box-filtered over each field texel) and a point-flare mask (bright taps minus their surround) into a reduced field whose divisor scales with Spread (4 to 16), blurs the glow with two separable 33-tap Gaussian iterations and streaks the flare mask with 65-tap exponential horizontal and vertical kernels, then maps the smooth field through the shared LUT with optional antialiased bands, fades the outer edge, restores the crisp shape with Keep Shape and mixes the flare color. `color-halos.mjs` checks styles and defaults, the editor and runtime passes on synthetic textures (Amount 0, paper above the threshold untouched, unchanged far from the shape, ramp core and rim, keep shape, bands, light shapes, flares along the axes), editor/runtime parity, history, duplication, save/reopen, shader export and runtime pixel parity, and renders every style (Gradient Maps on grouped text, Cross Flare on light points) into `.context/composition-test/color-halos-*.png`. Manual check: [COLOR-HALOS-MANUAL-QA.md](COLOR-HALOS-MANUAL-QA.md).
